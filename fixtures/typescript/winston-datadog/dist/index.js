...
var SERVICE = "orders-api";
var DD_SOURCE = "nodejs";
var DD_TAGS = "env:local,fixture:winston";
var RESERVED = /* @__PURE__ */ new Set(["level", "message", "timestamp", "stack"]);
...
var TEXT = import_winston.format.printf((info) => {
  const meta = extras(info).map(([key, value]) => `${key}=${renderValue(value)}`).join(" ");
  const level = info.level.toUpperCase().padEnd(5);
  return `${info.timestamp} ${level} ${info.message}${meta ? `  ${meta}` : ""}${frames(info.stack)}`;
});
var JSON_LINES = import_winston.format.printf((info) => {
  const { stack, errorKind, errorMessage, ...rest } = info;
  const line = {
...
function createLogger(shape) {
  return (0, import_winston.createLogger)({
    ...
    format: import_winston.format.combine(
      ...
      import_winston.format.timestamp({ format: () => (/* @__PURE__ */ new Date()).toISOString() }),
    ...
    transports: [new import_winston.transports.Console({ stderrLevels: [] })]
  });
}

// src/index.ts
function readFromStore(orderId) {
...
var handler = async (event) => {
  const shape = event.format === "json" ? "json" : "text";
