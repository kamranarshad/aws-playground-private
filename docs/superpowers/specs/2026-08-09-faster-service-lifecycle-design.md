...
3. **The toggle syncs the selection — with `stopNow` on uncheck.** Today the
   checkbox only PATCHes `localServices`: nothing starts on check (the next
   invoke errors with "not running"), and nothing stops on uncheck until a
   later selection change finally applies the grace. The fix: after the
   PATCH succeeds, the toggle triggers a selection sync — on check this
   starts the newly enabled service right away; on uncheck it passes
   `stopNow: true`, and `setSelection(needed, { stopNow })` stops
   auto-started services that fell out of the needed set immediately
   (concurrently, cancelling any pending grace timer) instead of scheduling
   the 15s grace. The response's `scheduledStop` reports them as today.
   - `server/api.js` `setSelection` passes `stopNow` from the request body.
   - `web/src/lib/api.ts` `setSelection` gains an optional `stopNow`
     parameter; `useSelectionSync`'s mutation input becomes
     `{ functionId, stopNow? }`. Only the uncheck path sends `stopNow`.
     Selection changes from switching functions and the unload beacon keep
     the 15s grace, so function-hopping still doesn't thrash slow-booting
     services.
   - The manual-service rule is unchanged: services the user started by
     hand are not in `autoStarted` and are never auto-stopped (`stop()`
     already owns the demote-and-cancel bookkeeping the immediate path
     needs).
