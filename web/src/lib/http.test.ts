...
it('defaults to a plain GET against the function root when the event has no APIGW shape', () => {
  expect(buildCurlCommand(fn, '{}', 9500)).toBe("curl -X GET 'http://localhost:9500/myfn/'")
})

it('falls back the same way when the event JSON does not parse', () => {
  expect(buildCurlCommand(fn, '{not json', 9500)).toBe("curl -X GET 'http://localhost:9500/myfn/'")
})
...
it('builds method, path, query string, and headers from an APIGW v2 payload', () => {
  ...
  expect(buildCurlCommand(fn, JSON.stringify(event), 9500)).toBe(
...
it('builds the query string from queryStringParameters when rawQueryString is absent', () => {
  ...
  expect(buildCurlCommand(fn, JSON.stringify(event), 9500))
...
it('percent-encodes the function name segment, since the listener decodes it when routing', () => {
  const event = { rawPath: '/hello', queryStringParameters: { name: 'world' } }
  expect(buildCurlCommand({ name: 'api gateway' }, JSON.stringify(event), 9500))
...
it('uses the port it is given rather than a baked-in constant', () => {
  expect(buildCurlCommand(fn, '{}', 9600)).toBe("curl -X GET 'http://localhost:9600/myfn/'")
})
